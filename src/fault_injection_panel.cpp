  ...
  rclcpp::Client<ros2_fault_injection::srv::ReloadScenario>::SharedFuture future)
{
  ...
  if (response->success) {
    selected_fault_.config_values[config_key_dropdown_->currentText().trimmed().toStdString()] =
      config_value_edit_->text().trimmed().toStdString();
    refresh();
  }
}
  ...
  rclcpp::Client<ros2_fault_injection::srv::GetFaultSchema>::SharedFuture future)
{
  ...
  config_key_dropdown_->clear();
  for (const auto & key : selected_fault_.config_keys) {
  ...
  config_set_label_->setText("Selected fault: " +
                               QString::fromStdString(selected_fault_.fault_id) +
                               " (injector: " +
      QString::fromStdString(selected_fault_.injector_id) +
                               ", type: " + QString::fromStdString(selected_fault_.injector_type) +
                               ")");
  ...
  rclcpp::Client<ros2_fault_injection::srv::GetFaultConfig>::SharedFuture future)
{
  ...
  const auto current_key = config_key_dropdown_->currentText().toStdString();
  if (selected_fault_.config_values.count(current_key) > 0) {
    config_value_edit_->setText(QString::fromStdString(selected_fault_.config_values[current_key]));
  } else {
    config_value_edit_->setText("");
  }
}
...
void FaultInjectionPanel::on_set_config_clicked()
{
  ...
  config_set_label_->setText("Updating " + QString::fromStdString(selected_fault_.fault_id) + ": " +
...
void FaultInjectionPanel::on_config_key_changed(const QString & key)
{
  ...
  if (selected_fault_.config_values.count(key_str) > 0) {
    config_value_edit_->setText(QString::fromStdString(selected_fault_.config_values[key_str]));
  } else {
    config_value_edit_->setText("");
  }
}
